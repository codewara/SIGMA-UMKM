...
export const umkmProfileSchema = z.object({
  ...
  owner_id: z.uuid().optional(), // Link to users collection
  ...
  pemilik: z.object({
    ...
    telepon: z.string()
      ...
      .regex(/^[0-9+]+$/, "Nomor telepon tidak valid").optional(),
    email: z.email("Email tidak valid").optional(),
  }).optional(),
  lokasi: z.object({
    type: z.literal("Point"),
    coordinates: z.array(z.number()).length(2, "Koordinat harus [lng, lat]"),
  }).optional(),
  wilayah: z.object({
    kota: z.string(),
    provinsi: z.string(),
