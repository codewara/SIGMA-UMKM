...
export default function AdminDashboard() {
  ...
  const [umkmList, setUmkmList] = useState([
    { id: 1, name: 'Toko Roti Berkah', location: 'Jakarta Timur', phone: '08123456789', email: 'roti@berkah.com', revenue: 250.5, status: 'Aktif', description: 'Toko roti tradisional yang berfokus pada bahan organik.' },
    { id: 2, name: 'Kerajinan Tangan Indah', location: 'Bandung', phone: '08234567890', email: 'kerajinan@indah.com', revenue: 180.2, status: 'Aktif', description: 'Produksi kerajinan tangan lokal kualitas ekspor.' },
    { id: 3, name: 'Konveksi Sejahtera', location: 'Surabaya', phone: '08345678901', email: 'konveksi@sejahtera.com', revenue: 95.8, status: 'Aktif', description: 'Konveksi pakaian seragam dan kaos komunitas.' },
  ]);
        ...
        <main className="flex-1 overflow-y-auto px-6 lg:px-8 pb-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
              {[
                { label: 'Total UMKM', val: umkmList.length, icon: <Users className="text-blue-400" /> },
                ...
                { label: 'Unit Aktif', val: '100%', icon: <MapPin className="text-orange-400" /> }
              ].map((s, i) => (
                    ...
                    {filteredUmkm.map((u) => (
                      <tr key={u.id} className="hover:bg-white/5 transition-colors group">
                        <td className="px-10 py-8">
                          <div className="text-white font-bold group-hover:text-blue-400 transition-colors">{u.name}</div>
                          <div className="text-white/30 text-xs mt-1 italic line-clamp-1">{u.description}</div>
                        </td>
                        <td className="px-10 py-8">
                          <div className="flex items-center text-white/60 text-sm">
                            <MapPin size={14} className="mr-2 text-white/20" /> {u.location}
                          </div>
                        </td>
                        <td className="px-10 py-8">
                          <span className="font-mono font-bold text-white">Rp {u.revenue}M</span>
                        </td>
                        <td className="px-10 py-8">
                          <div className="flex justify-center">
                            <span className="bg-green-500/10 text-green-400 border border-green-500/20 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tighter">
                              {u.status}
                            </span>
                          </div>
                        </td>
                        <td className="px-10 py-8">
                          <div className="flex justify-end gap-3">
                            <button onClick={() => {setSelectedUmkm(u); setShowModal(true)}} className="p-3 bg-white/5 hover:bg-white/20 border border-white/10 rounded-xl text-white transition-all">
                              <Eye size={18} />
                            </button>
                            <button className="p-3 bg-white/5 hover:bg-red-500/20 border border-white/10 rounded-xl text-red-400 transition-all">
                              <Trash2 size={18} />
                            </button>
                          </div>
                        </td>
