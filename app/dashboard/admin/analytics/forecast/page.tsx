...
export default function AdminForecastAnalytics() {
  ...
  const fetchForecastData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/analytics/forecast?scope=global');
      if (response.ok) {
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-blue-500/30 rounded-3xl p-6 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-white/70 text-sm mt-1">Forecast ini berbasis data historis 12 bulan terakhir dengan confidence score untuk setiap periode.</p>
          </div>
        </div>
