...
export default function PejabatHeatmapAnalytics() {
  ...
  const fetchHeatmapData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/analytics/heatmap?scope=region');
      if (response.ok) {
  ...
  return (
    <div className="space-y-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Heatmap Konsentrasi UMKM Regional</h1>
          <p className="text-white/70">Visualisasi distribusi UMKM di wilayah Anda</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-6">
              <p className="text-white/70 text-sm">Total UMKM Terdata Wilayah</p>
